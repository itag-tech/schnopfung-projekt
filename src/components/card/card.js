...
const CustomCard = ({ product }) => {
  ...
  const removeProductUnliked = productClicked => {
    const productsLikedFiltered = productsLiked.filter(
      productLiked => productLiked !== productClicked
    )
  ...
  const addProductLiked = productClicked => {
    const productsLikedArray = [...productsLiked]
  ...
  return (
    <Card
      ...
      actions={[
        ...
        productsLiked.includes(product) ? (
          <HeartFilled
            className="liked"
            onClick={() => handleProductLiked(product)}
          />
        ) : (
          <Icon
            type="heart"
